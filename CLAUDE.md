...
- **Penales en KO reales**: `wc_results` (Supabase) guarda sólo el marcador de 90'. Para no perder el ganador de un KO definido por penales se agregaron columnas `adv` (`'home'|'away'`), `home_pen`, `away_pen`, que la edge function `wc-sync` completa desde ESPN (`advance/winner` + `shootoutScore`) enriqueciendo la fila de TheSportsDB. El cliente las lee vía `sbToEvt()`; además `pollLiveScores` re-consulta a ESPN cualquier KO empatado sin ganador (`pendingKoBuckets`) como red de seguridad. Sin esto, un KO 1-1 quedaba con `adv=null` y no llenaba la llave siguiente.
- **Cuotas de partido** (`computeOdds` → `oddsHtml` grupos / `koOddsHtml` KO): son **1X2 estimadas del cruce** (modelo POW), marcadas `~est.`. NO derivar de las odds de campeón (`_bettingOdds`): eso mostraba un % de favoritismo global (ej. 95%/5%) que no es la chance del partido. Las odds de campeón se usan sólo en la sección de estadísticas.
